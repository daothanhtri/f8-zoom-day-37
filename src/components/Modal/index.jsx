...
const Modal = forwardRef(
  (
    ...
    const [_internalIsOpen, _setInternalIsOpen] = useState(isOpen);
    ...
    useEffect(() => {
      if (isOpen !== _internalIsOpen) {
        _setInternalIsOpen(isOpen);
      }
    }, [isOpen, _internalIsOpen]);
    ...
    useImperativeHandle(
      ...
      () => ({
        open: () => {
          _setInternalIsOpen(true);
        },
        close: () => {
          _setInternalIsOpen(false);

          onRequestClose && onRequestClose();
        },
        toggle: () => {
          _setInternalIsOpen((prev) => {
            const newState = !prev;
            if (!newState && onRequestClose) {
              onRequestClose();
            }
            return newState;
          });
      ...
      [onRequestClose, _setInternalIsOpen]
    );
    ...
    const overlayClasses = [
      styles.overlay,
      overlayClassName,
      _internalIsOpen && isTransitioning && styles.entering,
      !_internalIsOpen && isTransitioning && styles.exiting,
      _internalIsOpen ? styles.visible : styles.hidden,
    ]
      .filter(Boolean)
      .join(" ");
